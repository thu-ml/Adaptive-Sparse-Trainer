...
from llmshearing.models.metrics import DomainCount, DomainLanguageCrossEntropy, KL_Divergence
...
def flash_attn_fn(
    ...
    # flash attention version changed
...
class DistillComposerMosaicLlama(ComposerModel):
    """ Llama model with the Composer model interface. """
    def __init__(self, cfg):
        super().__init__()
        self.model = LlamaModel(cfg)
        self.teacher = LlamaModel(cfg)
        self.kl_weight = cfg.get('kl_weight', 0.5)
        self.kl_temp = cfg.get('kl_temp', 0.5)
        self.num_fwd_flops = self._compute_num_fwd_flops()
        self.train_metrics = {
            'LanguageCrossEntropy': LanguageCrossEntropy(),
            'Perplexity': LanguagePerplexity(),
    ...
    def forward(self, batch):
        ...
        if pruned_steps is not None:
            pruned_steps = pruned_steps[0].item()
        zs = {key: batch[key] for key in batch if "_z" in key}
        student_outputs = self.model(input_ids=input_ids, key_padding_mask=key_padding_mask, pruned_steps=pruned_steps, **zs)
        model_logits = student_outputs['logits']
        l0_outputs = student_outputs.get('l0_output', None)
        if self.training:
            with torch.no_grad():
                teacher_logits = self.teacher(input_ids=input_ids, key_padding_mask=key_padding_mask, pruned_steps=pruned_steps, **zs)['logits']
        else:
            teacher_logits = None
        outputs = {"logits": model_logits, "teacher_logits": teacher_logits, "l0_output": l0_outputs}
        return outputs
    ...
    def loss(self, outputs, batch):
        ...
        ce_loss = F.cross_entropy(student_logits.view(-1, student_logits.size(-1)),
                                   targets.view(-1),
                                   ignore_index=-100)
        return_loss = {"ce_loss": ce_loss}
        if l0_output is not None:
            lag_loss = l0_output[0]
            return_loss["lag_loss"] = lag_loss
        if teacher_logits is not None:
            temperature = self.kl_temp
            ...
            return_loss["kl_loss"] = self.kl_weight * kl_loss
        return_loss["total"] = sum(return_loss.values())
        return return_loss
    ...
    def update_metric(self, batch, outputs, metric) -> None:
        ...
        if isinstance(metric, DomainLanguageCrossEntropy):
            ...
            with torch.inference_mode():
                ...
                selected_sets = batch['set']
            metric.update(selected_sets, idx)
        elif isinstance(metric, KL_Divergence):
